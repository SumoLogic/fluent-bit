...
static struct tail_file_lines get_out_file_content(const char *target)
{
    ...
    int line_c=0;
    struct tail_file_lines file_lines = {};
...
static int cb_check_result(void *record, size_t size, void *data)
{
    ...
    if (!*streamfilter ||
        ...
        printf("Record doesn't match any expected line: %s\n", (char *)record);
    }
...
{
    do_test("tail", "dockermode", 2000, 3,
            NULL);
}

void flb_test_in_tail_dockermode_splitted_line()
{
    do_test("tail", "dockermode_splitted_line", 2000, 2,
            NULL);
}

void flb_test_in_tail_dockermode_multiple_lines()
{
    do_test("tail", "dockermode_multiple_lines", 4000, 2,
